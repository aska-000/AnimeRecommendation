...
public class BDAnime {
    ...
    public static void initSchema() throws SQLException {
        ...
        stat.execute("CREATE TABLE IF NOT EXISTS anime (" +
    ...
    public static void seedData() throws SQLException {
        ...
        Object[][] items = {
                ...
                {4, "Монстр", "Триллер", 8.7, "Доктор Кэндзо Тэнма — талантливый хирург, который работает в Германии. Однажды он спасает тяжело раненого мальчика. Этот случай послужит началом череды загадочных убийств, а спустя годы Кэндзо вновь столкнётся с тем, кого он когда-то спас от смерти.", "images/monster.jpg"},
                {5, "Семья шпиона", "Комедия", 8.5, "Герой этой истории, Лойд Форджер, является «Джеймсом Бондом» своего времени. Погони, шпионаж, перестрелки, тайные миссии — всё это является неотъемлемой частью его жизни. Закончив одно задание, он тут же принимается за другое — усталость ему неведома. Однако на этот раз миссия оказывается немного необычной: он должен сохранить мир между двумя странами, а для этого ему предстоит обзавестись фиктивной семьёй. Роль «роковой красотки из Бондианы», и по совместительству жены Лойда, достаётся Йор Форджер — профессиональной наёмной убийце с кодовым именем Тернистая Принцесса. Для полноты картины осталось обзавестись ещё и ребёнком, и эспер Аня Форджер только рада заполучить себе крутых новых родителей.\n" +
                ...
                {7, "Ван-Пис", "Приключения", 8.8, "Перед своей казнью легендарный пират Капитан Роджер открыл всему миру тайное место, где спрятал награбленные сокровища.\n" +
                ...
                {17, "Код Гиас", "Меха", 8.9, "Альтернативная реальность: Британская империя захватила треть мира, обрушивая на непокорных мощь своей военной машины.\n" +
        ...
        for (Object[] a : items) {
            ...
            ps.setDouble(4, (double) a[3]);
            ps.setString(5, (String) a[4]);
            ps.setString(6, (String) a[5]);
            ps.executeUpdate();
        }
        System.out.println("Ваши аниме загружены: 20 шт");
    }
    ...
    public static void syncWithAPI() throws IOException, InterruptedException, SQLException {
        ...
        String sql = "INSERT OR IGNORE INTO anime (id, title, genre, rating, description, image_path) VALUES (?, ?, ?, ?, ?, ?)";
        PreparedStatement ps = conn.prepareStatement(sql);
        ...
        for (Anime anime : apiAnimeList) {
            ...
            if (result > 0) {
                count++;
                System.out.println("Добавлено новое: " + anime.id + " - " + anime.title);
            }
        }
        System.out.println("Добавлено новых аниме из API: " + count);
    }
    ...
    public static ArrayList<Anime> getAllAnime() throws SQLException {
        ...
        rs = stat.executeQuery("SELECT * FROM anime ORDER BY id");
        while (rs.next()) {
            list.add(new Anime(
                    rs.getInt("id"),
                    rs.getString("title"),
    ...
    public static boolean isDatabaseEmpty() throws SQLException {
        ...
        ResultSet rs = st.executeQuery("SELECT COUNT(*) FROM anime");
        rs.next();
